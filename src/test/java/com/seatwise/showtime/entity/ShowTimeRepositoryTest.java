...
class ShowTimeRepositoryTest {
  ...
  void findShowsInDateRange() {
    ...
    ShowTime showTime1 = new ShowTime(event, null, date, startTime, endTime);
    ShowTime showTime2 = new ShowTime(event, null, date.plusDays(3), startTime, endTime);
    ShowTime showTime3 = new ShowTime(event, null, date.plusDays(4), startTime, endTime);
    showTimeRepository.saveAll(List.of(showTime1, showTime2, showTime3));

    // when
    List<ShowTime> datesInRange =
        showTimeRepository.findByEventIdAndDateBetween(event.getId(), date, date.plusDays(3));
  ...
  void findShowsByEventIdAndDate() {
    ...
    ShowTime showTime1 = new ShowTime(event, null, date, time, time.plusHours(1));
    ShowTime showTime2 = new ShowTime(event, null, date, time.plusHours(2), time.plusHours(3));
    ShowTime showTime3 =
        new ShowTime(event, null, date.plusMonths(1), time.plusHours(2), time.plusHours(3));
    showTimeRepository.saveAll(List.of(showTime1, showTime2, showTime3));

    // when
    List<ShowTime> showTimes =
        showTimeRepository.findShowTimesByEventIdAndDate(event.getId(), LocalDate.of(2025, 1, 1));
